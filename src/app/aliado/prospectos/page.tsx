...
export default function AliadorProspectos() {
  ...
  async function marcarEstado(asignacionId: string, prospectoId: string, resultado: string) {
    const supabase = createClient()
    await supabase
      .from("asignaciones")
      .update({ resultado, fecha_resultado: new Date().toISOString() })
      .eq("id", asignacionId)

    if (resultado === "cerrado") {
      await supabase.from("prospectos").update({ estado: "cerrado" }).eq("id", prospectoId)
    } else if (resultado === "no_cerrado") {
  ...
  return (
    <main className="p-4 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        ...
        <a href="/aliado/prospectos/nuevo" className="bg-gray-900 text-white text-sm px-4 py-2 rounded-xl">
          + Nuevo
        </a>
      </div>
            ...
            <div key={p.asignacion_id} className="bg-white rounded-2xl border border-gray-100 p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className={"text-xs px-2 py-1 rounded-full " + (
                  ...
                  {p.resultado === "cerrado" ? "Cerrado" :
                   p.resultado === "no_cerrado" ? "No cerrado" : "Activo"}
                </span>
              </div>
              ...
              {p.resultado === "activo" && (
                <div className="flex gap-2 mt-3">
                    ...
                    className="flex-1 flex items-center justify-center bg-green-500 text-white py-2 rounded-xl text-xs font-medium"
                  >
                    WhatsApp
                  </a>
                  
                    href={"/aliado/registrar-cierre?asignacion=" + p.asignacion_id}
                    className="flex-1 flex items-center justify-center bg-gray-900 text-white py-2 rounded-xl text-xs font-medium"
                  >
                    Registrar venta
                  </a>
                  <button
                    onClick={() => {
                      if (confirm("Confirmas que no pudiste cerrar este prospecto?")) {
                        marcarEstado(p.asignacion_id, p.id, "no_cerrado")
                      }
                    }}
                    className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-xl text-xs"
                  >
                    No cerré
                  </button>
                </div>
