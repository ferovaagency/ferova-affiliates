...
export default async function AdminDashboard() {
  const [
    ...
    adminClient.from('comisiones').select('monto_comision, aliados(nombre)').eq('estado', 'pendiente'),
    adminClient.from('ventas').select('*, aliados(nombre), servicios(nombre)').order('fecha_creacion', { ascending: false }).limit(10),
  ])

  const totalPendiente = comisionesPendientes?.reduce((s, c) => s + c.monto_comision, 0) ?? 0
    ...
    <main className="p-6 max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex gap-2">
          <a href="/admin/prospectos" className="bg-gray-900 text-white text-sm px-4 py-2 rounded-xl">
            + Prospecto
          </a>
          <a href="/admin/aliados" className="border border-gray-200 text-gray-700 text-sm px-4 py-2 rounded-xl">
            + Aliado
          </a>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className={`rounded-2xl p-4 border ${(prospectosSinAsignar ?? 0) > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
          <p className="text-xs text-gray-500 mb-1">Sin asignar</p>
          <p className={`text-2xl font-semibold ${(prospectosSinAsignar ?? 0) > 0 ? 'text-amber-700' : 'text-gray-900'}`}>
            {prospectosSinAsignar ?? 0}
          </p>
        </div>
        <div className={`rounded-2xl p-4 border ${totalPendiente > 0 ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-100'}`}>
          <p className="text-xs text-gray-500 mb-1">Comisiones por pagar</p>
          <p className={`text-xl font-semibold ${totalPendiente > 0 ? 'text-amber-700' : 'text-gray-900'}`}>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
        ...
        {ventasRecientes && ventasRecientes.length > 0 ? (
              ...
              <div key={v.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {(v.aliados as any)?.nombre} · {v.descripcion_servicio ?? (v.servicios as any)?.nombre ?? 'Servicio'}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold">{formatCOP(v.monto)}</p>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    v.estado === 'pagado' ? 'bg-green-50 text-green-700' :
                    v.estado === 'cancelado' ? 'bg-red-50 text-red-700' :
                    'bg-amber-50 text-amber-700'
                  }`}>
                    {v.estado === 'pagado' ? 'Pagado' :
                     v.estado === 'cancelado' ? 'Cancelado' : 'Pendiente pago'}
                  </span>
                </div>
              </div>
            ))}
          ...
          <div className="p-8 text-center">
            <p className="text-sm text-gray-400">No hay ventas todavía</p>
          </div>
        )}
      </div>
    </main>
